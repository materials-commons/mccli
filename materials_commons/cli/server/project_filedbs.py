...
class ProjectFileDBs:
    ...
    def get_filedb(self, project_id: int) -> FileIndexDB:
        if project_id not in self._filedbs_for_project:
            ...
            if not p:
                ...
                cwd = Path.cwd()
                p_path = project_path(cwd)
                self._filedbs_for_project[project_id] = FileIndexDB(Path(p_path) / ".mc" / "mc2.sqlite")
            else:
                self._filedbs_for_project[project_id] = FileIndexDB(Path(p["project_dir_path"]) / ".mc" / "mc2.sqlite")
