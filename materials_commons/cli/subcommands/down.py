...
def down_subcommand(argv, working_dir):
    ...
    if args.globus:
        ...
        asyncio.run(download_async_v2(proj, paths, db=None, recursive=args.recursive))
    else:
        output = None
        if args.output:
...
async def download_async_v2(proj, paths, db, recursive=False):
    ...
    print(f"paths = {paths} recursive = {recursive}")
    proj.remote.raise_exception = False

    print(f"proj.local_path = {proj.local_path}")

    try:
        async for batch in walk_remote_tree_async(proj.remote, proj.id, paths, recursive):
            print("\n========")
            print(f"directory {batch.directory_path}")
            local_path = Path(proj.local_path) / batch.directory_path.lstrip('/')
            print(f"local path: {local_path}")
            local_entries = await scan_local_directory_async(local_path.as_posix(), proj.local_path)
            for entry_path, entry_stat in local_entries.items():
                print(f"local entry: {entry_path}")
...
async def scan_local_directory_async(path: str, proj_path: str):
    results = {}
    project_path = Path(proj_path)
    entries = await aiofiles.os.scandir(path)
    for entry in entries:
        p = Path(entry.path)
        st = await aiofiles.os.stat(entry.path)
        results["/" + p.relative_to(project_path).as_posix()] = st
    return results
